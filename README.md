# Integração correios springboot + junit 5

## Badges
![GitHub Org's stars](https://img.shields.io/github/stars/gabriel2mm?style=social)

![GitHub Org's stars](http://img.shields.io/static/v1?label=STATUS&message=EM%20DESENVOLVIMENTO&color=GREEN&style=for-the-badge)

## Índice
* [Título](#title)
* [Badges](#Badges)
* [Índice](#índice)
* [Descrição do Projeto](#Descrição)
* [Pendências](#Pendências)
* [Licença](#Licença)

## Descrição

Este projeto foi utilizado como estudo para implementação de melhores práticas de desenvolvimento
e testes unitários utilizando springboot com junit 5. 

Para realização do  estudo foi implementado a integração com os serviços do correios busca de CEP.

## Autor

| [<img src="https://avatars.githubusercontent.com/u/16054681?v=4" width=115><br><sub>Gabriel Moré Maia</sub>](https://github.com/gabriel2mm) |
|:-------------------------------------------------------------------------------------------------------------------------------------------:|

## Pendências

- Implementação de documentação Swagger.

## Licença

Este projeto é licenciado pelo MIT licensed.